...
from utils import dice_emote, SYM_X, EmbedColor, bet_emoji, RattleType
...
class LiarCalledEmbed(discord.Embed):
    def __init__(self, liar: Liar, players: Dict[int, Player], show_actual = False):
        ...
        self.description = f"**{liar_player.name}** called liar on **{bid_player.name}**'s `{target_bid.quantity}` {SYM_X} {dice_emote(target_bid.pips)}"
        if show_actual:
            self.description += f"\nThere was actually `{liar.actual_quantity}` {SYM_X} {dice_emote(target_bid.pips)}"
...
class RoundSummaryEmbed(discord.Embed):
    ...
    def get_dice_counts_field(self):
        ...
        for face in range(1, 7):
            ...
            dice_counts.append(f'`{count}` {SYM_X} {dice_emote(face)}')
        return '\n'.join(dice_counts)

    def get_dice_totals_field(self):
        ones_count = self.all_dice.count(1)
        dice_totals = [f'`{ones_count}` {SYM_X} {dice_emote(1)}']
        for face in range(2, 7):
            count = self.all_dice.count(face) + ones_count
            dice_totals.append(f'`{count}` {SYM_X} {dice_emote(face)}')
        return '\n'.join(dice_totals)
