...
class PerudoContext(commands.Context):
    ...
    async def send_auction_message(self, auction: Auction):
        self.bot.active_message = await self.send_delayed(view=AuctionView(self), embed=AuctionEmbed(auction))
    
    async def send_round_message(self, round: Round):
        self.bot.active_message = await self.send_delayed(view=RoundView(self), embed=RoundEmbed(round))
    
    async def send_setup_message(self, setup: GameSetup):
        self.bot.active_message = await self.send_delayed(view=GameSetupView(self), embed=GameSetupEmbed(setup))
    ...
    async def _update_message(self, edit_function, **kwargs):
        edit_function = edit_function or self.bot.active_message.edit
        recent_history = [message async for message in self.channel.history(limit=2)]
        if self.bot.active_message in recent_history:
            await edit_function(**kwargs)
        else:
            await self.bot.active_message.delete()
            self.bot.active_message = await self.send_delayed(**kwargs, delay=0)
